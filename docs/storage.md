...
- RustFS's object data (`ansible/roles/rustfs/`), mounted from a CIFS share on the NAS (`//192.168.0.111/proxmox/data/rustfs`) — on the Proxmox host itself, bind-mounted into the LXC as `/mnt/nas/rustfs` (unprivileged LXCs can't mount CIFS/NFS themselves, see `services/rustfs/README.md`) — is **not** in this repo's backup scope, same reasoning as Jellyfin's media library — it's expected to be backed up at the NAS level, not re-backed-up through the LXC.
- Keycloak's Postgres data (`ansible/roles/keycloak/`), mounted from a CIFS share on the NAS (`//192.168.0.111/proxmox/data/keycloak`) the same way as RustFS — but unlike RustFS's bulk object data, this holds realms, users, and client configuration, the same category of irreplaceable state as the Obsidian vault. It must be covered by a NAS-level backup; see `services/keycloak/README.md`.
