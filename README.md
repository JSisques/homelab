...
* A single-node K3s server (`ansible/roles/k3s/`) with Argo CD bootstrapped on top, empty and ready for `Application` resources. Kafka (via Strimzi, under `kubernetes/`) is defined but deliberately not applied yet — see `kubernetes/argocd/applications/kafka.yaml`'s header comment for why. Gardenia (`kubernetes/applications/gardenia/`) is the first application-level Kubernetes workload, defined here first: `gardenia-web` + `gardenia-api` + Postgres, exposed publicly at `gardenia.sisqueslabs.com` via the existing Cloudflare Tunnel and k3s's built-in Traefik ingress — see that directory's `README.md` for the one-time manual steps (NFS export, secrets, DNS) still needed before it's actually live.
