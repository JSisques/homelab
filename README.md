...
* Ansible roles and playbooks for every LXC/VM service (`it-tools`, `n8n`, `obsidian`, `jellyfin`, `downloads`, `supabase`, `monitoring` [Prometheus + Grafana + Loki + Alertmanager], `homepage`, `uptime-kuma`, `cloudflared`, `adguard-home`, `wireguard`, `pbs`) plus a mandatory base (`common`, `node-exporter`, `promtail`, `docker`), driven by a generated inventory (`scripts/generation/generate-inventory.sh`).
...
* Self-hosted Supabase (`services/supabase/`) — Postgres plus Auth, REST, Realtime, Storage, and Edge Functions, fronted by Kong, all on one LXC. Gives other homelab services (n8n, future apps) a shared Postgres-backed backend instead of each bringing its own database. Postgres data and Storage buckets live on the NAS over NFS, same pattern as Obsidian and Jellyfin. LAN-only (`https://supabase.home.arpa`); see `services/supabase/README.md`.
