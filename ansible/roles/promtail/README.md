...
- Render `/etc/promtail/config.yml` pointing at Loki (`promtail_loki_url`, default `http://192.168.1.20:3100/loki/api/v1/push` — the `monitoring` LXC).
- Ship the systemd journal, labeled by host and unit.

## Variables

```yaml
promtail_loki_url: http://192.168.1.20:3100/loki/api/v1/push
