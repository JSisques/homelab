...
- Publicado en el host de la LXC `monitoring` en `192.168.0.209:4317` (gRPC) y `:4318` (HTTP) para que apps de la LAN puedan enviarle datos.
...
- Añadir un nuevo servicio instrumentado es solo apuntar su `OTEL_EXPORTER_OTLP_ENDPOINT` a `http://192.168.0.209:4318`.
