...
Las aplicaciones que emiten datos OpenTelemetry (hoy, `cookidoo-mcp`) envían trazas/métricas/logs OTLP al OTel Collector de la LXC `monitoring` (`192.168.1.20:4317`/`4318`), que los reparte: las métricas quedan expuestas en un endpoint que Prometheus scrapea (pull, no `remote_write`, así que la configuración de la instancia de Prometheus compartida no cambia), los logs van al endpoint OTLP nativo de Loki, y las trazas a Tempo. Esto mantiene intacto el modelo de Prometheus basado en scraping mientras da a los servicios nativos de OTel un sitio al que enviar datos.
